...
class Graph:
    ...
    def addEdge(self, id, source, target, weight="", start="", end="", label="", attrs=[]):
        self._edges[id] = Edge(self, id, source, target, weight, start, end, label, attrs=attrs)
...
class GexfImport:
    ...
    def msg_unexpected_tag(self, expected, got):
        print
        "Error : incorrect xml. Expected tag {expected}, not {got}.".format(expected=expected, got=got)
